...
class MultKAN(nn.Module):
    ...
    def fit(self, dataset, opt="LBFGS", steps=100, log=1, lamb=0., lamb_l1=1., lamb_entropy=2., lamb_coef=0., lamb_coefdiff=0., update_grid=True, grid_update_num=10, loss_fn=None, lr=1.,start_grid_update_step=-1, stop_grid_update_step=50, batch=-1,
              metrics=None, save_fig=False, in_vars=None, out_vars=None, beta=3, save_fig_freq=1, img_folder='./video', singularity_avoiding=False, y_th=1000., reg_metric='edge_forward_spline_n', display_metrics=None, a_coef=1.0, b_coef=1.0, c_coef=0.0):
        '''
        training
        ...
        def closure():
            ...
            pred = self.forward(x, singularity_avoiding=singularity_avoiding, y_th=y_th)

            train_loss = a_coef * loss_fn(pred, dataset['train_label'][train_id])
        ...
        for _ in pbar:
            ...
            if opt == "Adam":
                ...
                pred = self.forward(x, singularity_avoiding=singularity_avoiding, y_th=y_th)
                first_derivative = torch.autograd.grad(pred, x, 
                ...
                train_loss = a_coef * loss_fn(pred, dataset['train_label'][train_id]) + c_coef * loss_fn(second_derivative, dataset['train_deriv'][train_id,2:4]) + b_coef * loss_fn(first_derivative, dataset['train_deriv'][train_id,:2])
